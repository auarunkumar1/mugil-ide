...
import { mugilCompressOutput, mugilPostprocess } from "@/mugil"
      ...
      const handleEvent = Effect.fnUntraced(function* (value: StreamEvent) {
        switch (value.type) {
          ...
          case "tool-result": {
            ...
            const output = {
              ...rawOutput,
              // Mugil graft: RTK command-output compression on the tool result
              // text before it is fed back to the model (keeps error lines).
              output: mugilCompressOutput(
                omitted === 0
                  ? rawOutput.output
          ...
          case "text-end":
            ...
            // Mugil graft: post-generation watermark removal + AI code-signature
            // stripping on the finalized assistant text part.
            ctx.currentText.text = mugilPostprocess(ctx.currentText.text)
            {
              const end = Date.now()
