...
import { mugilFireWebhooks } from "@/mugil/webhooks"
    ...
    const state = yield* InstanceState.make<State>(
      Effect.fn("Plugin.state")(function* (ctx) {
        ...
        const unsubscribe = yield* events.listen((event) => {
          ...
          return Effect.sync(() => {
            // Mugil graft: map fork session events to mugil webhook events.
            const webhookEvent: string | undefined =
                ...
                : event.type === "session.next.step.ended"
                  ...
                  : event.type === "session.next.step.failed"
                    ...
                    : event.type === "session.next.tool.called" ||
                      ...
                      : undefined
            if (webhookEvent) {
              mugilFireWebhooks(webhookEvent, {
                event: event.type,
                sessionID: (event.data as Record<string, unknown> | undefined)?.sessionID,
